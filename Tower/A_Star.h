...
bool Path_Field::check_if_tile_is_inaccessible(int xdx, int ydy, int finish_x, int finish_y)
{
	...
	if (!(xdx == finish_x && ydy == finish_y))
	{
		...
		{
			if (world_pointer->world_tiles[xdx][ydy]->multi_tile_config.build_time >= 100 && world_pointer->world_tiles[xdx][ydy]->multi_tile_config.door_state == 0) inaccessible = true;
		}
		if (world_pointer->item_tiles[xdx][ydy] != NULL && world_pointer->item_tiles[xdx][ydy]->multi_tile_config.is_collidable == 1)
		{
			if (world_pointer->item_tiles[xdx][ydy]->multi_tile_config.build_time >= 100) inaccessible = true;
		}
	}
	return inaccessible;
}
